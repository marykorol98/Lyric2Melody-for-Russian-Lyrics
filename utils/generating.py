...
import re
from utils.generating_consts import LYRICS_GENERATING_PARAMS, CHORDS_GENERATING_PARAMS, CHORDS_WINDOW, LYRICS_PREFIX, \
    CHORDS_PREFIX #  CHORDS_BEST_VERSION, LYRICS_BEST_VERSION,
...
class SongWriter:
    def __init__(self, chords_model, tokenizer, lyrics_prefix=LYRICS_PREFIX, # lyrics_model
    ...
    @staticmethod
    def fix_write_chords_params(lyrics_list): # genres_list, 
        # if isinstance(genres_list, str):
        #     genres_list = [[genres_list]]
        ...
        assert isinstance(lyrics_list[0], str) #isinstance(genres_list[0][0], str) and 
        return lyrics_list # genres_list, 
    ...
    def write_song_chords(self, lyrics, chords_generating_params=None): # genres, 
    ...
    def write_songs_chords(self,  lyrics_list=List[str], lyrics_generating_params=None,
                           chords_generating_params=None): # genres_list=List[List[str]], 
        
        # genres_list, lyrics_list = SongWriter.fix_write_chords_params(genres_list, lyrics_list)
        lyrics_list = SongWriter.fix_write_chords_params(lyrics_list)
        ...
        for lyrics in lyrics_list:
            songs.append(self.write_song_chords(lyrics, chords_generating_params))
        return songs
...
def get_best_song_writer():
    ...
    song_writer = SongWriter(chords_model, tokenizer, verbose=True) # lyrics_model
    return song_writer
