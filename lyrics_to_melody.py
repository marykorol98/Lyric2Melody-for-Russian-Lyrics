...
custom_lm = TransformerLanguageModel.from_pretrained('music-ckps/', 'checkpoint_best.pt', tokenizer='space',
                                                     batch_size=8192).cuda() ##batch_size=8192
...
def save_demo(notes_str, select_chords, name, lang, sentence, word_counter):
    ...
    cnt = 0
    sen_idx = 0
    print('select_chords length : ', len(select_chords))
...
if __name__ == '__main__':
    ...
    with open(lyrics_path) as lyrics_file, open(chord_path) as chord_file:
        ...
        for lyrics, chord in zip(lyrics_corpus, chord_corpus):
            ...
            lang = chord[:2]
            assert lang in ['zh', 'en', 'ru']  # Note that ROC is not language-sensitive, you can extend this.
            ...
            save_demo(note_string, select_chords, name, lang,
                      lyrics.strip().split('[sep]') if lang == 'en' or lang == 'ru' else sentence, [len(i) for i in sentence])
